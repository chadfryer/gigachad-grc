...
export class EmailTemplatesService {
  ...
  /**
   * SECURITY: Encode HTML entities to prevent XSS/HTML injection in emails.
   * User-controlled content must be escaped before interpolation into HTML templates.
   */
  ...
  /**
   * SECURITY: Sanitize URL to prevent javascript: and data: URL injection
   */
  ...
  private controlStatusChanged(
    ...
    const status = this.encodeHtml(String(metadata?.status || 'Unknown'));
    const safeTitle = this.encodeHtml(title);
    const safeMessage = this.encodeHtml(message);
    const controlLink = this.sanitizeUrl(`${this.baseUrl}/controls/${metadata?.controlId || ''}`);
    ...
    return {
      ...
      html: this.wrapHtml(
        ...
        <p>The status of control <strong>${safeTitle}</strong> has been changed to <strong>${status}</strong>.</p>
        <p>${safeMessage}</p>
  ...
  private defaultTemplate(
    ...
    // SECURITY: Encode user-controlled content to prevent HTML injection
    const safeTitle = this.encodeHtml(title);
    const safeMessage = this.encodeHtml(message);
