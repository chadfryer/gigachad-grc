...
export function handleError(
  ...
  logger: Logger,
  context?: string,
  // Fail-safe: Only include stack traces if explicitly enabled AND not in production
  includeStack = process.env.SHOW_STACK_TRACES === 'true' && process.env.NODE_ENV !== 'production'
): ErrorDetails {
...
export function createApiErrorResponse(error: unknown, correlationId?: string): ApiErrorResponse {
  ...
  return {
    ...
    error: {
      ...
      // Fail-safe: Only include details if explicitly enabled AND not in production
      details:
