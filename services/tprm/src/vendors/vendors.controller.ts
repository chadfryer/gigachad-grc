...
export class VendorsController {
  ...
  findAll(
    ...
    // SECURITY: Filter by organizationId to prevent IDOR - users can only see their org's vendors
  ...
  getDashboardStats(@CurrentUser() user: UserContext) {
    // SECURITY: Filter by organizationId to prevent IDOR - stats scoped to user's organization
  ...
  getDocumentAnalysis(
    ...
    // SECURITY: Pass organizationId to validate document belongs to user's organization
    return this.vendorAIService.getPreviousAnalysis(documentId, vendorId, user.organizationId);
  }
